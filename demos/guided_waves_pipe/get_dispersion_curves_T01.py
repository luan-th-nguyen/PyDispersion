...
def read_ascii(path, NR, nt):
    ...
    for rec_x in range(0,NR):
        ...
        ## rotate 90 degree
        #rotation = np.array([[np.cos(np.pi/2), -np.sin(np.pi/2)], [np.sin(np.pi/2), np.cos(np.pi/2)]])
        #xy = rotation.dot(xy)
...
if __name__ == '__main__':
    ...
    path = '../../data/guided_waves_pipe/data_T01/'
    NR = 24
    nt = 2000 # 6000
    ...
    ## slant stack method
    #p,tau,U_ptau = dispersion.slant_stack(u,dx,cmax,fmax)
    #img,f = np.abs(dispersion.fft(U_ptau,dt,nt))
    #df = f[1] - f[0]
    ...
    #plt.show()
    im.savefig('./pipe_syn_dispersion_curves_T01'  +  '.png',dpi=300)
