...
path_in = '../../data/guided_waves_pipe/data_L01/'
path_out = './'
...
for rec_x in range(rec_sta+1,rec_fin+1):
    ...
    #print np.max(xz2[:,1])
    plt.plot(offset[index] + 0.5e3*data_plot[index,:], time, 'b-', linewidth=2.0) # obs. radial component
...
#plt.show()
plt.savefig(path_out + 'traces_supershot_dispersion_analysis_L01'  +  '.png',dpi=300)
